/**
 * @file cards_ncurses.c
 * @version 1.0
 * @date 22/03/2025
 * @author Julien Martinez
 * @brief Ce fichier permet de tester l'affichage des cartes tirées aléatoirement
 * @result Fonctionnel
 * @todo \
...
int main(void)
{
    ...
    char names[MAX_PLAYERS][NAME_MAX_LENGTH] = {
        "Julien",
        "Simon",
        "Player 3",
        "Player 4",
    ...
    */
    ...
    for (j = 0; j < 2; j++)
    {
        for (i = 0; i < 4; i++)
        {
            ...
            char label[32];
            print_text_in_window(winBlock, 1, 1, answers[i + 4 * j], blockWidth);
            wrefresh(winBlock);
        }
        startY += blockHeight;
    }
